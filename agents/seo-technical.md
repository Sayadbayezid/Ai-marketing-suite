...
### LCP (Largest Contentful Paint) â Loading
| Rating | Threshold |
|--------|-----------|
| Good | <2.5s |
| Needs Improvement | 2.5â4s |
| Poor | >4s |

**LCP Subparts:**
- **TTFB** (Time to First Byte): target <800ms
- **Resource Load Delay**: time between TTFB and resource start â target <100ms
- **Resource Load Duration**: download time for LCP resource â target <700ms
- **Element Render Delay**: time from download to paint â target <50ms

### INP (Interaction to Next Paint) â Interactivity
| Rating | Threshold |
|--------|-----------|
| Good | <200ms |
...
- Check for redirect loops (AâBâA)
- HTTPâHTTPS redirect must exist
